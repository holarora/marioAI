...
//	private LearningTask task = null;
	...
	private String[] stageParams = {
		"-lde on -ltb off -ld 2 -ls 0 -le g",  // MainTask4_1.java
		"-lco off -lb on -le off -lhb off -lg on -ltb on -lhs off -lca on -lde on -ld 5 -ls 133829",  // MainTask4_2.java
		"-lde on -i off -ld 30 -ls 133434 -lhb on"  // MainTask4_3.java
	};
...
//	private EvaluationInfo localEvaluationInfo;
	...
	public void learn(){
		...
		for(int generation=0 ; generation<evaluationQuota ; generation++){
			...
			System.out.println("世代Gen : "+generation);

			/* 100個体の評価 */

			compFit(stageParams[0]);
			compFit(stageParams[1]);
			compFit(stageParams[2]);
			Arrays.sort(agents);
			...
			/* boost mutation rate if stuck */
			initialDistance = finalDistance;
			finalDistance = agents[0].getDistance();;
			...
			if(
				agents[0].getDistance() > 500 &&
				agents[0].getFitness() > fmax
			){
				bestAgent = (Agent)agents[0].clone();	//bestAgentを更新
				fmax = agents[0].getFitness();	//fmax更新
			...
			// if (evaluationInfo.distancePassedCells)
	 ...
	 */
	private void compFit(String params) {
		...
		for(int i=0 ; i<popsize ; i++){
			...
			if (params == "-lde on -ltb off -ld 2 -ls 0 -le g") {
				agents[i].setFitness(evaluationInfo.distancePassedCells + evaluationInfo.coinsGained);
				agents[i].setDistance(evaluationInfo.distancePassedCells);
			} else {
				agents[i].setFitness(agents[i].getFitness() + evaluationInfo.distancePassedCells + evaluationInfo.coinsGained);
				agents[i].setDistance(agents[i].getDistance() + evaluationInfo.distancePassedCells);
			}
			...
			// 		basicTask.setOptionsAndReset(marioAIOptions);
			// 		/* ステージ生成 */
			// 		String stageParam = stageParams[j];
			// 		marioAIOptions.setArgs(stageParam);
			...
			// 		} else {
			// 			/* 評価値(距離)をセット */
			// 			totalDistance += basicTask.getEvaluationInfo().distancePassedCells;
			// 			totalCoins += basicTask.getEvaluationInfo().coinsGained;
			// 		}
			// 	}
			// // agents[i].setFitness(evaluationInfo.distancePassedCells);
			// agents[i].setFitness(totalDistance + totalCoins);
			// agents[i].setDistance(totalDistance);
...
//		System.out.println("Aselect : " + (float)(Acount)/(Acount+Bcount)*100 +"\n"+"Bselect:"+(float)(Bcount)/(Acount+Bcount)*100);
	...
	/* 突然変異 */
	private void mutate(float rateofMutate) {

		int popsize2 = popsize - 2;
//		float mutation = popsize * mutateRate;	//突然変異させる個体数(float型)を設定(現在10個体)
		int mutationInt = (int) Math.floor(popsize*rateofMutate);	//突然変異させる個体数(int型)
//		float mutation3 = (1 << 16) * mutateRate;	//突然変異させる遺伝子座の個数(float型)
		int mutateGeneInt = (int) Math.floor((1<<16) * rateofMutate);	//突然変異させる遺伝子座の個数(int型)(65536)
